...
void drawBrickCube()
{
    if (gl_FrontFacing) {
		...
		vec4 MatlDiffuse = texture(BrickDiffuseMap, v2fTexCoord.xy);
		...
		FragColor = LightAmbient * MatlDiffuse +
					 LightDiffuse * MatlDiffuse * N_dot_L +
					 LightSpecular * vec4(BrickSpecularMaterial,1) * pf;
...
void drawWoodenCube()
{
    if (gl_FrontFacing) {
		...
		if (sqrDist >= MirrorRadius*MirrorRadius) {
			vec4 MatlDiffuse = texture(WoodDiffuseMap, v2fTexCoord.xy);
			...
			FragColor = LightAmbient * MatlDiffuse +
						 LightDiffuse * MatlDiffuse * N_dot_L +
						 LightSpecular * (WoodSpecularMaterial,1) * pf;
		} else {
			vec3 B, T;
			compute_tangent_vectors(necNormal, ecPosition, v2fTexCoord.xy, T, B);
			...
			vec4 wcReflectVec = inverse(ViewMatrix) * vec4(ecReflectVec,0);
			FragColor = texture(EnvMap, wcReflectVec.xyz);
		}
    }
    else discard;
}
